...
Pipeline üç ana kaynaktan beslenir:
...
3. **SEC EDGAR:** NVIDIA'nın resmi şirket bildirimleri (10-K, 10-Q, 8-K).
...
*   **Argümanlar:** `--source` (all, fred, gdelt, edgar) ve `--clean` (eski verileri silme) parametreleri alır.
...
### 3.3. SEC EDGAR (Resmi Şirket Finansal Raporları)
...
1.  Python ortamında `requests`, `pandas`, `beautifulsoup4`, `lxml`, `python-dotenv` kütüphanelerinin yüklü olması gerekir (`requirements.txt` dosyasından kurulabilir).
