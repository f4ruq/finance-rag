...
def run(use_blob: bool = False):
    ...
    if use_blob:
        ...
        logger.info(f"YFinance insight raw blob: {CONTAINER_RAW}/{insight_blob}")
        
        news_blob = BLOB_PATHS["yfinance"].format(filename=news_filename)
        upload_json(CONTAINER_RAW, news_blob, detailed_news_data)
        logger.info(f"YFinance news raw blob: {CONTAINER_RAW}/{news_blob}")
        
        # Clean/processed data'yı clean container'a da kaydet (RAG için)
        upload_json(CONTAINER_CLEAN, insight_blob, data)
        upload_json(CONTAINER_CLEAN, news_blob, detailed_news_data)
        logger.info(f"YFinance data clean container'a da kaydedildi")
