...
class FredCollector:
    ...
    def run(self):
        ...
        if self.use_blob:
            ...
            blob_name_raw = BLOB_PATHS["fred_summary"].format(filename=report_filename)
            self.save_json(blob_name_raw, report, blob_container=CONTAINER_RAW)
            
            # Clean/summary data'yı clean container'a da kaydet (RAG için)
            blob_name_clean = BLOB_PATHS["fred_summary"].format(filename=report_filename)
            self.save_json(blob_name_clean, report, blob_container=CONTAINER_CLEAN)
            print(f"Saved to both containers: {blob_name_clean}")
        else:
